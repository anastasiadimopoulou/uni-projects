...
struct Box
{
    ...
    */
    ...
    float intersectUp(Box& other) {
        if (fabs(m_pos_x - other.m_pos_x) * 2.0f >= (m_width + other.m_width) || m_pos_y < other.m_pos_y) {
            return 0.0f; // No upward intersection
        }
        else {
            float upwardOverlap = m_pos_y - m_height / 2.0f - (other.m_pos_y + other.m_height / 2.0f);
            return (upwardOverlap < 0.0f) ? fabs(upwardOverlap) : 0.0f;
        }
    }
    ...
    */
    Box(float x, float y, float w, float h) : m_pos_x(x), m_pos_y(y), m_width(w), m_height(h) {}
